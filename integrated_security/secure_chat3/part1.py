...
def main():
    ...
    seen = set()
    sharon_username = None

    print("[+] Waiting for Sharon's username...")
    for _ in range(300):
        ...
        for m in pts:
            if m not in seen:
                seen.add(m)
                print("[dec]", m)
                # Look specifically for Sharon's username pattern
                if "it was '" in m:
                    sharon_username = re.search(r"it was '([^']+)'", m).group(1)
                    print(f"[+] Found Sharon's username: {sharon_username}")
                    break
        if sharon_username:
            break
        time.sleep(1)
    else:
        print("[-] Sharon's username not found after timeout.")
        return
